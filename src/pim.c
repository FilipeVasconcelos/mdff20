...
void induced_moment_inner_kO(double (*mu_ind)[3], double (*ef_ext)[3]){
    ...
    tol=sqrt(fabs(log(eps*4.0*cutlongrange)));
...
void find_min(double (*ef0)[3], double (*ef1)[3],double (*mu0)[3], double (*mu1)[3]){ 
    ...
    for(int ia=0;ia<nion;ia++){
        ...
        if (!lpolar[it]) continue;
        for (int i=0;i<3;i++){
            ef0sq      += ef0[ia][i] * ef0[ia][i];
            ef0ef1mef0 += ef0[ia][i] * (ef1[ia][i] - ef0[ia][i]) ;
        }
    }
    /* alpha = - ef0^2 / ( ef0 ( ef0 - ef1 ) )  */
    alpha = - ef0sq / ef0ef1mef0 ;
    if ( fabs(ef0ef1mef0) < 1e-10) alpha = 1.0;

    for(int ia=0;ia<nion;ia++){
        for (int i=0;i<3;i++){
            ef0[ia][i] +=   alpha * ( ef1[ia][i] - ef0[ia][i] );
            mu0[ia][i]  =   alpha * ( mu1[ia][i] - mu0[ia][i] ); 
        }
    }
}
...
/* as the electric dipole are shorter ranged copare to charges                */
/******************************************************************************/
void momentpolaSCFkO2(double (*mu_ind)[3],double *upol){
    ...
    ef_stat=malloc(nion*sizeof(*ef_stat));
    ef_ind=malloc(nion*sizeof(*ef_ind));
    ef_prev=malloc(nion*sizeof(*ef_prev));
    f_ind=malloc(nion*sizeof(*f_ind));
    dmu_ind=malloc(nion*sizeof(*dmu_ind));
    mu_prev=malloc(nion*sizeof(*mu_prev));
    for(int ia=0;ia<nion;ia++){
