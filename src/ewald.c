...
/******************************************************************************/
/* Ewald Summation */
void multipole_ES(double *q, double (*mu)[3], double (*theta)[3][3],double *u, double (*ef)[3], double (*efg)[3][3]){
    ...
    *u= (u_dir + u_rec + u_self ) * coul_unit;
    ...
    for (int i=0;i<3;i++){
        for (int j=0;j<3;j++){
            tau_coul[i][j] = ( tau_dir[i][j] + tau_rec[i][j] ) * coul_unit / press_unit;
        }
    }
...
void multipole_ES_dir(double *q, double (*mu)[3], double (*theta)[3][3], 
    ...
    #pragma omp parallel shared(cutsq,rx,ry,rz,q,mu,theta,lqchtask,ldiptask) \
                         private(ia,j1,jb,je,ja,qi,qj,qij,mui,muj,rij,rxi,ryi,rzi,fxij,fyij,fzij,d2,T0,T1,T2,T3, F0, F1, F2, F3, F4, F5) 
    {
        #pragma omp for reduction (+:uu,ttau,fx_dir[:nion],fy_dir[:nion],fz_dir[:nion],tef[:nion],tefg[:nion]) schedule(dynamic,16) 
    ...
    *u_dir = uu; 
    ...
    for(int i=0; i<3;i++){
        for(int j=0; j<3;j++){
            tau_dir[i][j] =  ttau[i][j] * simuCell.inveOmega * 0.5;
        }
    }
    /*
    printf("tau_dir\n");
...
void multipole_ES_rec(double *q, double (*mu)[3], double (*theta)[3][3],
    ...
    double rxi,ryi,rzi;
    double fxij,fyij,fzij;
    double qi,k_dot_r,k_dot_mu;
    //double *ckr,*skr;
    double ckria,skria;
    double rhonk_R,rhonk_I,str;
    double recarg,recarg2;
    ...
    #pragma omp parallel shared(kcoul,rx,ry,rz,q,mu,theta) \
                         private(ik,qi,kx,ky,kz,kk,Ak,kcoe,rxi,ryi,rzi,k_dot_r,k_dot_mu,rhonk_R,rhonk_I,str,fxij,fyij,fzij,ckria,skria)
    { 
        #pragma omp for reduction (+:uu,ttau,fx_rec[:nion],fy_rec[:nion],fz_rec[:nion]) schedule(dynamic,32) 
    ...
    */
            ...
            ttau[1][0] +=         - kcoe * ky * kx   * str;
            ttau[1][1] +=   ( 1.0 - kcoe * ky * ky ) * str;
            ttau[1][2] +=         - kcoe * ky * kz   * str;
            ttau[2][0] +=         - kcoe * kz * kx   * str;
            ttau[2][1] +=         - kcoe * kz * ky   * str;
    ...
    *u_rec =   uu ;
    ...
    for (int ia=0;ia<nion;ia++){
        ...
        fz_rec[ia] *= 2.0 * fpiV;
        for (int j=0;j<3;j++){
            ef_rec[ia][j] *= 2.0 * fpiV;
            for (int k=0;k<3;k++){
                efg_rec[ia][j][k] *= 2.0 * fpiV;
            }
        }
    }
    ttau[1][0]=ttau[0][1];
    ...
    for (int j=0;j<3;j++){
        for (int k=0;k<3;k++){
            tau_rec[j][k] = ttau[j][k] * 2.0 * tpiV * simuCell.inveOmega;
        }
    }
    /*
    printf("tau_rec\n");
